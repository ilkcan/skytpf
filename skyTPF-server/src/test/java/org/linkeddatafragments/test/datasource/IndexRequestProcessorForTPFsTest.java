...
import org.linkeddatafragments.datasource.index.IndexRequestProcessor;
import org.linkeddatafragments.datasource.index.IndexTPFWorker;
...
public class IndexRequestProcessorForTPFsTest {
  ...
  static class TestIndexRequestProcessor extends IndexRequestProcessor {
    ...
    protected TestWorker getTPFSpecificWorker(
      ...
      TestWorker testWorker = new TestWorker(request);
      testWorker.setModel(model);
      return testWorker;
    }

    class TestWorker extends IndexTPFWorker {
      public TestWorker(final ITriplePatternFragmentRequest<RDFNode, String, String> req) {
        super(req);
      }
    }
  }
  ...
  /**
   * Check that the type void:Dataset is returned as a URI and not as a literal
   */
  @Test
  public void shouldGiveVoidDatasetAsAURI() {
    ...
    final TestIndexRequestProcessor processor = new TestIndexRequestProcessor(baseUrl, datasources);
    final TriplePatternElementFactory<RDFNode, String, String> factory =
        new TriplePatternElementFactory<RDFNode, String, String>();
    final ITriplePatternFragmentRequest<RDFNode, String, String> request =
        new TriplePatternFragmentRequestImpl<RDFNode, String, String>(null, "dummy", false, 1,
            factory.createUnspecifiedVariable(), factory.createUnspecifiedVariable(),
            factory.createUnspecifiedVariable());

    final TestIndexRequestProcessor.TestWorker worker = processor.getTPFSpecificWorker(request);
    ...
    final Resource object = ResourceFactory.createResource("http://rdfs.org/ns/void#Dataset");
    final Statement expected = ResourceFactory.createStatement(subject, predicate, object);
    Assert.assertTrue("triple not contained in model", statements.contains(expected));
    processor.close();
  }
}
