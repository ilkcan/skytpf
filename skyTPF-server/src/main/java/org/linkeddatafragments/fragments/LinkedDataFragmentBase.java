...
/**
 * Base class of any implementation of {@link ILinkedDataFragment} that uses paging.
 *
...
public abstract class LinkedDataFragmentBase implements ILinkedDataFragment {
   ...
   * @param isLastPage
   */
  protected LinkedDataFragmentBase(final String fragmentURL, final String datasetURL,
  ...
  /**
   * Does nothing. May be overridden by subclasses that hold some objects that need to be closed
   * (such as iterators from the underlying data source).
   */
  @Override
  public void close() {}
  ...
  /**
   * This implementation uses {@link #addMetadata(Model)}, which should be overridden in subclasses
  ...
  /**
   * This implementation uses {@link #addControls(Model)}, which should be overridden in subclasses
   * (instead of overriding this method).
   * 
   * @return
   */
  ...
  /**
   * Adds an RDF description of page links to the given RDF model. This method may be overridden in
   * subclasses.
   * 
   * @param model
   */
  public void addControls(final Model model) {
    ...
    final Resource firstPageId = model.createResource(
        pagedURL.setParameter(ILinkedDataFragmentRequest.PARAMETERNAME_PAGE, "1").toString());

    fragmentId.addProperty(CommonResources.HYDRA_FIRSTPAGE, firstPageId);

    if (pageNumber > 1) {
      final String prevPageNumber = Long.toString(pageNumber - 1);
      final Resource prevPageId = model.createResource(pagedURL
          .setParameter(ILinkedDataFragmentRequest.PARAMETERNAME_PAGE, prevPageNumber).toString());
    ...
    if (!isLastPage) {
      ...
      final Resource nextPageId = model.createResource(pagedURL
          .setParameter(ILinkedDataFragmentRequest.PARAMETERNAME_PAGE, nextPageNumber).toString());
