 ...
 * 
 * Ported by <a href="mailto:tzellman@gmail.com">Tom Zellman</a>. Extended by
 * <a href="mailto:miel.vandersande@ugent.be">Miel Vander Sande</a>
 * 
 */
public final class MIMEParse {
  ...
  /**
   * Find the best match for a given mimeType against a list of media_ranges that have already been
   * parsed by MimeParse.parseMediaRange(). Returns a tuple of the fitness value and the value of
   * the 'q' quality parameter of the best match, or (-1, 0) if no match was found. Just as for
   ...
   * parsed by parseMediaRange(). Returns the 'q' quality parameter of the best match, 0 if no match
   * was found. This function bahaves the same as quality() except that 'parsed_ranges' must be a
   ...
   * in header. The value of header must be a string that conforms to the format of the HTTP Accept:
  ...
  public static String bestMatch(List<String> supported, String header)
    ...
    for (String r : StringUtils.split(header, ',')) {
      ...
      } // Fail silently on malformed media type and omit from list.
    }
